...
class CI_Cart {
	...
	function _insert($items = array())
	{
		...
		// Prep the price.  Remove anything that isn't a number or decimal point.
		$items['purchaserate'] = trim(preg_replace('/([^0-9\.])/i', '', $items['purchaserate']));
		// Trim any leading zeros
		$items['purchaserate'] = trim(preg_replace('/(^[0]+)/i', '', $items['purchaserate']));

		// Is the price a valid number?
		if ( ! is_numeric($items['purchaserate']))
		{
			log_message('error', 'An invalid purchaserate was submitted for product ID: '.$items['id']);
			return FALSE;
		}
		...
		// Prep the price.  Remove anything that isn't a number or decimal point.
		//$items['packagename'] = trim(preg_replace('/([^0-9\.])/i', '', $items['packagename']));
		// Trim any leading zeros
		//$items['packagename'] = trim(preg_replace('/(^[0]+)/i', '', $items['packagename']));
