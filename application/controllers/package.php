...
class Package extends CI_Controller {
    ...
    function index()  {
        $data['title'] = "Package";
        $data['content'] = $this->load->view('package/add_package', $data, TRUE);
    ...
    function package_Insert()  {
        ...
        $query = $this->db->query("SELECT * from sr_package where package_name = '$name'");
        if($query->num_rows() > 0){
            ...
            $lastid = $this->mt->save_date_id("sr_package", $data);
        ...
        $this->load->view('package/add_package');
        }
    }
   
    function packageEdit() {
        $id = $this->input->post('editid');
        $query = "SELECT sr_package.*, sr_productcategory.* FROM sr_package left join sr_productcategory on sr_productcategory.ProductCategory_SlNo =sr_package.package_categoryid WHERE sr_package.package_ID = '$id'";
        $data['selected'] = $this->mt->edit_by_id($query);
        $this->load->view('edit/package_edit', $data);
    }
    function packageUdate(){
        ...
        $this->mt->update_data("sr_package", $data, $id,"package_ID");
        $this->load->view('package/add_package'); 
    } 
    function packageDelete(){
        $fld = 'package_ID';
        $id = $this->input->post("deleted");
        $this->mt->delete_data("sr_package", $id, "package_ID");
        $this->load->view('package/add_package');
    }
    // Careate Package=======================================================================
    function create()  {
        $data['title'] = "Package Create";
        $data['content'] = $this->load->view('package/create', $data, TRUE);
        $this->load->view('index', $data);
    } 
    function package_create_Insert()  {
        $name = $this->input->post('packageItem');
        $query = $this->db->query("SELECT * from sr_package_create where create_item = '$name'");
        if($query->num_rows() > 0){
           echo "F";
            ...
            $lasID = $this->mt->save_date_id("sr_package_create", $data);
            $data = array(
    ...
    function createDelete(){
        ...
        $this->mt->delete_data("sr_package_create", $id, 'create_ID');
        $this->mt->delete_data("tbl_product", $prodcid, 'Product_SlNo');
        $this->load->view('package/create');
    }
    function createEdit() {
        $id = $this->input->post('editid');
        $query = "SELECT sr_package_create.*, sr_package.*,tbl_product.*,sr_productcategory.* FROM sr_package_create left join sr_package on sr_package.package_ProCode = sr_package_create.create_pacageID left join tbl_product on tbl_product.product_create_pack_id=sr_package_create.create_ID left join sr_productcategory on sr_productcategory.ProductCategory_SlNo=tbl_product.ProductCategory_ID WHERE sr_package_create.create_ID = '$id'";
        $data['selected'] = $this->mt->edit_by_id($query);
        $this->load->view('edit/package_create_edit', $data);
    }
    function createUdate(){
        ...
        $pcode = $this->input->post('pcode');
            ...
            $this->mt->update_data("sr_package_create", $data, $id,'create_ID');
