...
<div class="content_scroll" style="padding:120px 20px 25px 160px">
    <a style="cursor:pointer" onclick="window.open('<?php echo base_url(); ?>quotation/sales_invoice', 'newwindow', 'width=850, height=800,scrollbars=yes'); return false;"><img src="<?php echo base_url(); ?>images/printer.png" alt=""> Print</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <a href="<?php echo base_url(); ?>Quotation" title="" class="buttonAshiqe">Back To Quotation</a>

    <?php
    $sql = mysql_query("SELECT sr_quotationmaster.*, sr_quotationmaster.AddBy as served, tbl_quotaion_customer.* FROM sr_quotationmaster left join tbl_quotaion_customer on tbl_quotaion_customer.quotation_customer_id = sr_quotationmaster.SalseCustomer_IDNo where sr_quotationmaster.SaleMaster_SlNo = '$SalesID'");
                ...
                <table width="100%">
                    ...
                    if($cusID == 0){
                        ...
                        <tr>
                            <td><strong>Customer ID </strong></td>
                            <td>:</td>
                            <td><?php echo $selse['SalseCustomer_IDNo']; ?></td>
                        </tr>
                        <tr>
                            <td><strong>Customer Name </strong></td>
                            <td>:</td>
    ...
    <table class="border" cellspacing="0" cellpadding="0" width="70%">
        ...
        $packageName = 0;
        $Ptotalamount = 0;
        $ssql = mysql_query("SELECT sr_quotationdetails.*, tbl_product.*  FROM sr_quotationdetails left join tbl_product on tbl_product.Product_SlNo = sr_quotationdetails.Product_IDNo where sr_quotationdetails.SaleMaster_IDNo = '$SalesID'");
        while($rows = mysql_fetch_array($ssql)){
            ...
            <?php } }
        $ssqls = mysql_query("SELECT sr_quotationdetails.*, tbl_product.*  FROM sr_quotationdetails left join tbl_product on tbl_product.Product_SlNo = sr_quotationdetails.Product_IDNo where sr_quotationdetails.SaleMaster_IDNo = '$SalesID' group by sr_quotationdetails.packageName");
        ...
        <tr>
            ...
            <td style="border:0px" align="right"><?php $paid = $selse['SaleMaster_PaidAmount']; echo number_format($paid,2);?></td>
