...
<a style="cursor:pointer" onclick="window.open('<?php echo base_url(); ?>reports/sales_challan', 'newwindow', 'width=850, height=800,scrollbars=yes'); return false;"><img src="<?php echo base_url(); ?>images/printer.png" alt=""> Print</a>

<?php
  $sql = mysql_query("SELECT sr_challanmaster.*, sr_challanmaster.AddBy as served, sr_customer.* FROM sr_challanmaster left join sr_customer on sr_customer.Customer_SlNo = sr_challanmaster.Customer_IDNo where sr_challanmaster.ChallanMaster_SlNo = '$ChallanMasterID'");
...
?>
        ...
        <tr>
            ...
            <td>
                <table width="100%">
                    ...
                    <tr>
                        <td><strong>Delivery Inv. No </strong></td>
                        <td>:</td>
    ...
    <table class="border" cellspacing="0" cellpadding="0" width="70%">
        ...
        $packageName ="";
        $Ptotalamount = "";
        $ssql = mysql_query("SELECT sr_challandetails.*, tbl_product.*  FROM sr_challandetails left join tbl_product on tbl_product.Product_SlNo = sr_challandetails.Product_IDNo where sr_challandetails.ChallanMaster_SlNo = '$ChallanMasterID'");
        ...
        <?php } }
            $ssqls = mysql_query("SELECT sr_challandetails.*, tbl_product.*  FROM sr_challandetails left join tbl_product on tbl_product.Product_SlNo = sr_challandetails.Product_IDNo where sr_challandetails.ChallanMaster_SlNo = '$ChallanMasterID' group by sr_challandetails.packageName");
